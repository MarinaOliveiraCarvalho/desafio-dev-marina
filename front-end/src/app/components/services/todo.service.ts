...
export class TodoService {
	...
	private link_file: String = `/core/file`;
	...
	public getStoreOwner(): Observable<any> {
		...
		return this.http.get<Array<TodoList>>(`${this.urlBase}${this.link_file}`,{ headers }).pipe(
			map((res) => {
	...
	uploadFile(file: File): Observable<HttpEvent<any>> {
		...
		const req = new HttpRequest('POST', `${this.urlBase}${this.link_file}`, formData, {
		  reportProgress: true,
		  responseType: 'json',
		  headers: headers
		});
